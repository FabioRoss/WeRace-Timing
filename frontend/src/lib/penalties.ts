 ...
 * Adjustments are signed: a time correction "+24s" / "−10s", a lap correction
 * "+2 laps" / "−1 lap". */
export function penaltyLabel(p: Penalty): string {
  if (p.kind === 'warning') return 'Warning'
  if (p.kind === 'lap') return `−${p.laps} lap${p.laps === 1 ? '' : 's'}`
  if (p.kind === 'adjust') {
    if (p.laps) return `${p.laps >= 0 ? '+' : '−'}${Math.abs(p.laps)} lap${Math.abs(p.laps) === 1 ? '' : 's'}`
    return `${p.seconds >= 0 ? '+' : '−'}${Math.abs(p.seconds)}s`
  }
  return `+${p.seconds}s`
}
...
export function penaltyKindLabel(p: Penalty): string {
  ...
  if (p.kind === 'adjust') return p.laps ? 'Lap adjustment' : 'Time adjustment'
  return 'Time penalty'
}
...
export function penaltyAdjustedDrivers(snapshot: Snapshot | null): DriverRow[] {
  ...
  // `laps` accumulates a SIGNED net delta: lap penalties subtract, lap
  // adjustments add their signed value (give laps back / remove).
  ...
  for (const p of snapshot?.penalties ?? []) {
    ...
    if (p.kind === 'lap') a.laps -= p.laps
    else if (p.kind === 'adjust') a.laps += p.laps
    add.set(p.kart_no, a)
  }
