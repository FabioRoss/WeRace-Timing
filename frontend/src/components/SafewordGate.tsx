...
export function SafewordGate({ children }: { children: React.ReactNode }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      clearSafeword()
      setError(t('Wrong safeword'))
    }
  }

  if (state === 'open') return <>{children}</>
  if (state === 'checking') {
    return <div className="grid h-full place-items-center text-ink-500">{t('Checking access…')}</div>
  }
  return (
    <div className="grid h-full place-items-center p-6">
      <form onSubmit={submit} className="w-full max-w-xs space-y-4 rounded-xl bg-pit-850 p-6">
        ...
        <h1 className="text-lg font-bold uppercase tracking-widest text-center">{t('Restricted')}</h1>
        <input
          type="password"
          autoFocus
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder={t('Safeword')}
