...
export function TimesheetPanel({
  ...
  const saveConfig = () => {
    ...
    void Promise.resolve(onSaveConfig(currentConfig())).then(() => {
      setSavedConfig(t('Saved as public default ✓'))
    ...
    <div className="max-w-2xl space-y-4">
      <div className="rounded-xl bg-pit-900 p-5 ring-1 ring-pit-800">
        <h2 className="text-sm font-bold uppercase tracking-wider text-ink-300">
          {t('Chrono timesheet (PDF)')}
        </h2>
        <p className="mt-1 text-sm text-ink-500">
          {t('A modern classification sheet with an optional lap-by-lap grid for every kart — the print-style sheet you\'d hand out after the race.')}
        </p>

        <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-3">
          <Summary label={t('Event')} value={eventName || '—'} />
          <Summary label={t('Track')} value={trackName || '—'} />
          <Summary label={t('Karts')} value={kartCount ? String(kartCount) : '—'} />
          <Summary label={t('Leader')} value={leaderName ? `#${leaderKart} ${leaderName}` : '—'} />
          <Summary
            label={t('Fastest lap')}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="label-race">{t('Event name')}</span>
            <input
              value={eventOverride}
          ...
          <label className="block">
            <span className="label-race">{t('Session name')}</span>
            <input
        ...
        <p className="mt-1 text-[0.65rem] text-ink-500">
          {t('Used on the sheet header and the file name (with the date).')}
        </p>

        <label className="mt-4 block">
          <span className="label-race">{t('Result status')}</span>
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value)}
            className="mt-1 w-full rounded bg-pit-950 px-3 py-2 text-sm ring-1 ring-pit-600 focus:ring-race-red sm:w-64"
          >
            <option value="">{t('Auto (from session state)')}</option>
            <option value="provisional">{t('Provisional')}</option>
            <option value="definitive">{t('Definitive')}</option>
          </select>
          <span className="mt-1 block text-[0.65rem] text-ink-500">
            {t('Stamped as a badge in the sheet header.')}
          </span>
        </label>

        <label className="mt-4 block">
          <span className="label-race">{t('Notes')}</span>
          <textarea
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            rows={3}
            maxLength={2000}
            placeholder={t('Optional notes printed on the sheet (e.g. stewards\' decisions, conditions)…')}
            className="mt-1 w-full rounded bg-pit-950 px-3 py-2 text-sm ring-1 ring-pit-600 focus:ring-race-red"
          />
        </label>

        <div className="mt-4 space-y-2">
          <div className="label-race">{t('Include')}</div>
          ...
          {pits && !autoPitlane && (
            <label className="ml-6 flex items-center gap-2 text-sm text-ink-300">
              ...
              />
              {t('Estimate stop durations (inferred)')}
            </label>
          )}
          <label className="flex items-center gap-2 text-sm">
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            {t('Apply penalties (final result)')}
          </label>
          {penalties && (
            <p className="ml-6 text-[0.65rem] text-ink-500">
              {t('Recomputes the classification with outstanding penalties applied and adds a summary.')}
            </p>
          )}
        </div>

        <div className="mt-4">
          <div className="label-race mb-1.5">{t('Accent colour')}</div>
        ...
        {onSaveConfig && (
          <p className="mt-2 text-[0.65rem] text-ink-500">
            {t('The public results page downloads the PDF with this layout.')}
          </p>
        )}
        {!kartCount && (
          <p className="mt-3 text-xs text-ink-500">
            {t('No timing data yet — connect a source (or replay a recording) first.')}
          </p>
        )}
      </div>
    </div>
